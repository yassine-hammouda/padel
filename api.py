...
model = joblib.load('models/xgb_classifier_matches.pkl')
df_ref = pd.read_csv('data/clean_matches.csv')
...
def save_predictions():
    ...
    filepath = r'C:\Users\hammo\Downloads\BI_ML_Project\outputs\predictions.csv'
...
def retrain():
    try:
        ...
        m.fit(X, y)
        joblib.dump(m, 'models/xgb_classifier_matches.pkl')
...
def predict_duration():
    try:
        model_reg = joblib.load('models/rf_regressor_matches.pkl')
        data = request.json
        row = pd.DataFrame([{
            'category_enc':   le_cat.transform([data['category']])[0],
            'round':          data['round'],
...
def cluster():
    try:
        kmeans  = joblib.load('models/kmeans_matches.pkl')
        scaler  = joblib.load('models/scaler_matches.pkl')
        data = request.json
        row = pd.DataFrame([{
            'category_enc':   le_cat.transform([data['category']])[0],
            'round':          data['round'],
    ...
    try:
        kmeans  = joblib.load('models/kmeans_matches.pkl')
        pca     = joblib.load('models/pca_matches.pkl')
        scaler  = joblib.load('models/scaler_matches.pkl')
        data = request.json
        row = pd.DataFrame([{
            'category_enc':   le_cat.transform([data['category']])[0],
        ...
        cluster_id = kmeans.predict(pca_result)[0]
        return jsonify({
            'cluster':      int(cluster_id),
            'cluster_name': f'Cluster {cluster_id}',
            'match_id':     data.get('match_id', 'N/A'),
...
def forecast():
    try:
        ...
        if len(series) < 2:
            ...
            counts = [int(s['count']) for s in series]
